...
/// Zhongshi Jiang, Scott Schaefer, Daniele Panozzo, ACM Trancaction on Graphics
/// (Proc. SIGGRAPH Asia 2017) For a complete implementation and customized UI,
/// please refer to https://github.com/jiangzhongshi/scaffold-map
struct SCAFData {
...
/// @param[in] bc          #b by dim list of boundary conditions (soft
/// constraint)
...
scaf_precompute(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
                const Eigen::MatrixXd &V_init, triangle::SCAFData &data,
                MappingEnergyType slim_energy, Eigen::VectorXi &b,
                Eigen::MatrixXd &bc, double soft_p);

IGL_INLINE void scaf_precompute_joint(
...
/// @returns resulting V_o (in SLIMData): #V by dim list of mesh vertex
/// positions
...
/// @param[in] s:   igl::SCAFData. Will be modified by energy and Jacobian
/// computation.
/// @param[out] L:   m by m matrix
/// @param[out] rhs: m by 1 vector
///         with m = dim * (#V_mesh + #V_scaf - #V_frame)
IGL_INLINE void scaf_system(triangle::SCAFData &s,
